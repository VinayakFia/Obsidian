...
short src_ln0 [] = {
	  0,  14,  15,  15,  16,  16,  18,  18, 
	 19,  19,  20,  20,  21,  21,  22,  23, 
	 23,  17,  25,  25,  25,  13,  27,  27, 
	  0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "calculator.pml", 1, 23 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 1:	/* :init: */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->me = addqueue(calling_pid, 1, 1);
		((P1 *)pptr(h))->result = 0;
#ifdef VAR_RANGES
		logval(":init::result", ((P1 *)pptr(h))->result);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* calc */
		...
		((P0 *)pptr(h))->lft = 0;
		((P0 *)pptr(h))->rgt = 0;
		((P0 *)pptr(h))->me = addqueue(calling_pid, 2, 1);
#ifdef VAR_RANGES
		logval("calc:s", ((P0 *)pptr(h))->s);
		logval("calc:lft", ((P0 *)pptr(h))->lft);
		logval("calc:rgt", ((P0 *)pptr(h))->rgt);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.f = addqueue(calling_pid, 3, 0);
#ifdef VAR_RANGES
#endif
}
...
#endif
	switch (n) {
	case 3: j = sizeof(Q3); q_flds[3] = 2; q_max[3] = 12; break;
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.f) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.f) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.f) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.f) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P0 *)_this)->you) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P0 *)_this)->you) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P0 *)_this)->you) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P0 *)_this)->you) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P0 *)_this)->me) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P0 *)_this)->me) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P0 *)_this)->me) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P0 *)_this)->me) )))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P1 *)_this)->me) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P1 *)_this)->me) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P1 *)_this)->me) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P1 *)_this)->me) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  operator:	2\n");
	printf("	mtype  value:	1\n");
	printf("	chan f (=%d):	len %d:\t", now.f, q_len(now.f));
	c_chandump(now.f);
}
void
...
{	/* int i; */
	...
	printf("	chan me (=%d):	len %d:\t", ((P1 *)pptr(pid))->me, q_len(((P1 *)pptr(pid))->me));
	c_chandump(((P1 *)pptr(pid))->me);
	printf("	int    result:	%d\n", ((P1 *)pptr(pid))->result);
		break;
	case 0:
		printf("local vars proc %d (calc):\n", pid);
	printf("	chan you (=%d):	len %d:\t", ((P0 *)pptr(pid))->you, q_len(((P0 *)pptr(pid))->you));
	c_chandump(((P0 *)pptr(pid))->you);
	printf("	int    s:	%d\n", ((P0 *)pptr(pid))->s);
	printf("	int    lft:	%d\n", ((P0 *)pptr(pid))->lft);
	printf("	int    rgt:	%d\n", ((P0 *)pptr(pid))->rgt);
	printf("	chan me (=%d):	len %d:\t", ((P0 *)pptr(pid))->me, q_len(((P0 *)pptr(pid))->me));
	c_chandump(((P0 *)pptr(pid))->me);
		break;
	}
}
void
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("value"); return;
	case 2: Printf("operator"); return;

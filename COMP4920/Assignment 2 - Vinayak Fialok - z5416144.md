...
To explain some event or state $x$ through causal explanation is to specify the causes of $x$. Causal explanation itself appears in several forms of which we will discuss constant conjunction and counterfactual. In *constant conjunction causal* explanation we say that "(event or state) A causes (event or state) B when the two always occur together" (C. M. Lorkowski). This model as discussed by Lorkowskie not reasonably justified. *Counterfactual causal* explanation takes the form "if A had not occurred, B would not have occurred".

...
Grayson states that accuracy allows COMPAS to achieve fairness, however, fairness is defined not in terms of the outcome, but in terms of the reasoning. For predictive A
Earlier we defined the property of fairness as dependant on the reasoning of a decision. For predictive automated decision making, we must also ensure high accuracy of prediction to maintain fairness.



COMPAS scores a mean accuracy of 66% whereas CORELS achieves 66.5% (Angelino et al 2024 p40). Studies show that "COMPAS (and so CORELS) is not significantly better than the crowd" (Derssel J, Farid H, 2018). However, fairness is defined not in terms of outcome but of reasons. For recidivism prediction to be fair it must have fair reasoning that leads to an accurate outcome.

Moreover, COMPAS and CORLES both shown to have biases, particularly in race (Angelino et al 2024 p40).
